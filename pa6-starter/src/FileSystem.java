...
public class FileSystem {
    ...
    public boolean add(String fileName, String directory, String modifiedDate) {
        ...
        if (nameMap.get(fileName) == null){
            ...
            FileData oldFile = findFile(fileName, directory);
            if (oldFile != null && oldFile.equals(file)){
                return false;
            }else if (oldFile != null && oldFile.lastModifiedDate.compareTo(modifiedDate) < 0){
                sameFileDiffDate = true;
                sameFileDiffDateFile = oldFile;
                oldDate = oldFile.lastModifiedDate;
            }
            if (sameFileDiffDate){
        ...
        if (oldDate != null && dateMap.containsKey(oldDate)){
            ...
            dateMap.set(modifiedDate, dateFileList);
        }
        
        if (dateMap.get(modifiedDate) == null){
            ArrayList<FileData> dateList = new ArrayList<FileData>();
            dateList.add(file);
            dateMap.set(modifiedDate, dateList);
        } else if (oldDate != null && dateMap.containsKey(oldDate)){
            ArrayList<FileData> oldDateFileList = dateMap.get(oldDate);
            oldDateFileList.remove(sameFileDiffDateFile);
            if (oldDateFileList.isEmpty()){
                dateMap.remove(oldDate);
    ...
    public ArrayList<FileData> findFilesByDate(String modifiedDate) {
    	...
    	if (dateMap.containsKey(modifiedDate)){
            ArrayList<FileData> getList = nameMap.get(modifiedDate);
            if (getList == null){
                return fileDataALToReturn;
            }
            else{
